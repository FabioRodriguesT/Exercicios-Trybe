...
const booksQuantity = (object) => {
  if (object.favoriteBooks.length === 1) {
    return `${object.name} tem ${object.favoriteBooks.length} livro favorito.`;
  } else {
    return `${object.name} tem ${object.favoriteBooks.length} livros favoritos`;
  }
}

console.log("\n" + booksQuantity(reader));